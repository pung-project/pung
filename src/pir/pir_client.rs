...
extern "C" {
    ...
    fn cpp_client_generate_query(client: *const libc::c_void,
                                 index: libc::uint64_t,
                                 q_len: *mut libc::uint64_t,
                                 q_num: *mut libc::uint64_t)
                                 -> *mut libc::uint8_t;

    fn cpp_client_process_reply(client: *const libc::c_void,
    ...
    fn cpp_client_update_db_params(client: *const libc::c_void,
                                   len: libc::uint64_t,
                                   num: libc::uint64_t,
                                   alpha: libc::uint64_t,
                                   depth: libc::uint64_t);
...
impl<'a> PirClient<'a> {
    pub fn new(size: u64, num: u64, alpha: u64, depth: u64) -> PirClient<'a> {
        ...
        let client_ptr: &'a mut libc::c_void = unsafe { &mut *(cpp_client_setup(size * num, num, alpha, depth)) };

        PirClient { client: client_ptr, depth: depth }
    }
    ...
    pub fn gen_query(&self, index: u64) -> PirQuery<'a> {
        ...
        PirQuery { query: query, num: q_num }
    }
    ...
    pub fn decode_answer(&self, answer: &[u8], a_num: u64) -> PirResult<'a> {
        ...
        let result: &'a mut [u8] = unsafe {
            let ptr =
                cpp_client_process_reply(self.client, answer.as_ptr(), answer.len() as u64, a_num, &mut r_len);
