...
extern "C" {
    ...
    fn cpp_server_process_query(server: *const libc::c_void,
...
impl<'a> PirServer<'a> {
    pub fn new<T>(collection: &[T], alpha: u64, depth: u64) -> PirServer<'a> {
        ...
        let server_ptr: &'a mut libc::c_void =
            unsafe {
                &mut *(cpp_server_setup((collection.len() * mem::size_of::<T>()) as u64,
    ...
    pub fn gen_answer(&self, query: &[u8], q_num: u64) -> PirAnswer<'a> {
        ...
        let answer: &'a mut [u8] = unsafe {
            let ptr = cpp_server_process_query(self.server,
                                               query.as_ptr(),
                                               query.len() as u64,
                                               q_num,
                                               &mut a_len,
        ...
        PirAnswer { answer: answer, num: a_num }
    }
}
