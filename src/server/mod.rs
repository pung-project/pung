...
fn accept_loop(listener: gjio::SocketListener,
    ...
    listener.accept().then(move |stream| {
        ...
        let mut network =
            twoparty::VatNetwork::new(stream.clone(), stream, rpc_twoparty_capnp::Side::Server, reader_options);
...
pub fn run_rpc(addr: SocketAddr,
               worker: Root<Generic>,
    ...
    gj::EventLoop::top_level(move |wait_scope| -> Result<(), capnp::Error> {
            ...
            let connection =
                pung_rpc::ToClient::new(PungRpc::new(worker, send, dbase, extra_tuples, min_messages, opt_scheme))
